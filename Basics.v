...
Proof.
  intros p.
  rewrite <- mult_n_Sm.  (* Apply the lemma mult_n_Sm with m = 0 *)
  simpl.                  (* This simplifies p * S 0 to p * 0 + p, then to 0 + p *)
  rewrite <- mult_n_O.       (* Now, apply mult_n_O to rewrite p * 0 to 0 *)
  reflexivity.            (* Finish the proof, as 0 + p simplifies to p *)
Qed.
